...
export function OnboardingChecklistCard({
  ...
  function dismiss() {
    // Instant, client-only dismiss (persisted to localStorage). The smooth exit
    // is handled by AnimatePresence below; on failure there is nothing to roll
    // back because no server request is involved.
    setDismissed(true);
  ...
  // Hidden once dismissed or once every step is complete. AnimatePresence keeps
  // the card mounted just long enough to animate the collapse on the way out.
  const visible = !dismissed && !checklist.allComplete;
          ...
          <div className="rounded-[var(--radius-lg)] border border-[var(--line)] bg-white">
            ...
            <div className="flex items-start justify-between gap-4 px-6 pt-5 pb-4">
              ...
              <button
                type="button"
                onClick={dismiss}
                className="admin-focus shrink-0 rounded-full border border-[var(--line)] px-3 py-1 text-xs font-semibold uppercase tracking-[0.14em] text-[var(--ink-400)] transition hover:bg-[var(--sand-50)] hover:text-[var(--ink-700)]"
              >
                Dismiss
              </button>
            </div>
              ...
              {steps.map((step, index) => (
                <li key={step.id} className="flex items-center gap-4 rounded-[var(--radius-md)] px-4 py-3.5">
                  ...
                  {!step.complete && (
                    <Link
                      href={step.href}
                      className="admin-focus shrink-0 rounded-[var(--radius-sm)] border border-[var(--line)] bg-white px-3 py-1.5 text-xs font-semibold text-[var(--ink-700)] transition hover:bg-[var(--sand-50)] hover:text-[var(--ink-900)]"
                    >
                      {step.cta} →
                    </Link>
                  )}
